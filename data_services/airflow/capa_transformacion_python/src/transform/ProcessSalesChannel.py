###
# Script para procesar la información sobre canal de ventas.
# Salidas exit:
...
def cargar_dfs() -> tuple[DataFrame]:
    ...
    '''
    # Se carga el dataset
    df = cargar_dataset(dir_csv, extension='xlsx')

    #Se verifica que no falten columnas
    columnas_correctas =  ['CODIGO', 'DESCRIPCION']
...
def aplicar_metodos_generales_df(df) -> DataFrame:
    ...
    rellenar_nulos = {'CODIGO' : DESCONOCIDO_INT, 'DESCRIPCION' : DESCONOCIDO}
...
def almacenar_errores(df) -> DataFrame:
    # Se cambia después el nombre de la columna CODIGO por IdCanal
    CASOS = [('CODIGO', 'Error_CODIGO', '[^\\d]', 'integer', DESCONOCIDO_INT)
            ]
    
    df['Error_CODIGO'] = DESCONOCIDO
...
def agregar_columnas(df) -> DataFrame:
    df['IdCanal_Descartado']=0
    df['IdCanal_Correcto']=df['CODIGO']
    return df

# Paso 4
def comprobar_columnas_finales(df) -> DataFrame:
    columnas_correctas = ['CODIGO', 'DESCRIPCION', 'IdCanal_Actualizado', 
                        'IdCanal_Descartado', 'IdCanal_Correcto', 'Error_IdCanal']
...
def main():
    try:
        ...
        df.rename({'CODIGO_Actualizado' : 'IdCanal_Actualizado',
                   'Error_CODIGO' : 'Error_IdCanal'}, 
                    axis=1, inplace=True)
        # Paso 3
        ...
        df.rename({'CODIGO' : 'IdCanal' , 'DESCRIPCION' : 'Descripcion'},
                  inplace=True, 
                  axis=1)
        ...
        ERROR = type(e)
        print(f'ERROR ::: ({ERROR} no se pudo procesar el DataFrame de canal de ventas \nERROR:{e})')
        exit(4)
